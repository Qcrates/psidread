#' @title Construct the structure data frame
#' @description
#' The `psid_str()` function simplifies the creation of a structured data frame containing the user's desired variables.
#' This function draws inspiration from the psidtools package in STATA.
#' To utilize it, users need to provide either a string vector (which is recommended) or a single value (if copying and pasting from their STATA .do file).
...
#' Please refer to the details section for guidance on the string value format.
#'
...
#' - The recommended approach for providing the variable list is to separate them into distinct string values and then wrap them within a vector.
#' For example: `c(" hh_age || [13]ER53017  [17]ER66017", " p_age || [13]ER34204")`
#' `hh_age` and `p_age` are self-defined variable name. It depends on you! The final data output will show this variable name instead of the variable code like `ER34204`.
#' The `[YY]VARCODE` sequence can be found from the codebook of PSID. You do not need to make any changes on them.
#' Please ensure proper separation between the user-defined variable name and the `[YY]VARCODE` using "||".
#' Each variable sequence should be placed in a separate string.
...
#' Simply copy your STATA code into a single string value without making any alterations.
#' For instance: `"|| hh_age /// [13]ER53017  [17]ER66017 /// || p_age /// [13]ER34204///"`
#' Note the different position of "||" between the two input methods.
#' If your STATA code runs successfully, it should also function well here.
...
#'                 || p_age ///
#'                 [13]ER34204///" # DO NOT CHANGE YOUR STATA CODE
